...
def hard_level_constructor(loader, node):
    data = loader.construct_mapping(node)
    map = HardLevel.Map()
    objects = HardLevel.Objects()
    objects.config = data
    return {"map": map, "obj": objects}


yaml.add_constructor("!easy_level", easy_level_constructor)
yaml.add_constructor("!medium_level", medium_level_constructor)
yaml.add_constructor("!hard_level", hard_level_constructor)
